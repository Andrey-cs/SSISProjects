...
class StudentInfo(ttk.Toplevel):
    ...
    def create_button_callback(self):
        ...
        current_data = {
            ...
            "FIRSTNAME": firstname, 
            "LASTNAME": lastname, 
            # "SEX" will be handled based on mode.
            # If editing, it should not change from self.data.
            # If new, it takes selected_sex.
            "PROGRAM": selected_program, 
            "YEAR LEVEL": selected_year_level
        }

        # Explicitly handle SEX based on mode to ensure it's fixed in edit mode
        if self.mode == 'edit' and self.data:
            current_data["SEX"] = self.data.get("SEX", "") # Use the original sex data
        else: # New mode
        ...
        if self.mode == 'new':
            ...
            changed = False
            if self.data:
                # Compare current_data (excluding potentially fixed sex if it wasn't in self.data for some reason)
                # with self.data.
                for key in ["FIRSTNAME", "LASTNAME", "PROGRAM", "YEAR LEVEL"]: # Check only editable fields for changes
                    if self.data.get(key, "") != current_data[key]:
                        changed = True
                        break
                # Also check if ID (which is fixed) still matches, just in case.
                if self.data.get("ID") != current_data["ID"]:
                    changed = True # Should not happen as ID is disabled

            else: # Should not occur if self.data is always set for edit mode
                changed = True 

            if not changed: # If only sex could have changed but it's fixed, and others didn't change...
                 return self.dialog("No changes were made to editable fields.", "Information")

            # current_data already contains the original sex if in edit mode.
